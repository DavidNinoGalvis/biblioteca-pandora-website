...
export default function Navbar() {
  ...
  const handleLogout = () => {
    ...
    try {
      window.dispatchEvent(new CustomEvent("bp_user:logout"));
    } catch (e) {}
    router.push("/");
  };

  // Listen to global login/logout events so navbar updates when other components change session
  useEffect(() => {
    const onLogin = (e: any) => {
      try {
        const user = e?.detail;
        ...
        console.error("bp_user:login handler error", err);
      }
    };
    ...
    window.addEventListener("bp_user:login", onLogin as EventListener);
    window.addEventListener("bp_user:logout", onLogout as EventListener);

    return () => {
      window.removeEventListener("bp_user:login", onLogin as EventListener);
      window.removeEventListener("bp_user:logout", onLogout as EventListener);
    };
  }, []);
  ...
  const assignChallenge = async () => {
    ...
    try {
      // Primero verificar si ya tiene un reto activo
      const checkResponse = await fetch(`/api/challenges/active?userId=${user.id}`);
      const checkData = await checkResponse.json();

      if (checkData.activeChallenge) {
        // Ya tiene un reto activo, redirigir directamente
        console.log("Usuario ya tiene un reto activo, redirigiendo...");
        router.push("/reto");
        return;
      }
      ...
      if (response.ok) {
        ...
        localStorage.setItem(CHALLENGE_TYPE_KEY, chosenType);
        localStorage.setItem(CHALLENGE_KEY, JSON.stringify(selectedChallenge));
        router.push("/reto");
      } else {
        const errorData = await response.json();
        console.error("Error al asignar reto:", errorData);
        alert(`Error al asignar reto: ${errorData.error || 'Error desconocido'}`);
      }
    } catch (error) {
      console.error("Error al asignar reto:", error);
      alert("Error de conexión al asignar el reto");
    }
  };

  const inTeachersSection =
    !!pathname &&
    (pathname.startsWith("/admin") || pathname.startsWith("/profesores"));
            ...
            <div className="flex items-center gap-2 md:gap-3">
              {!user ? (
                ...
                <motion.button
                  ...
                  className="bg-white/90 hover:bg-white text-gray-800 px-3 md:px-4 py-2 rounded-lg font-bold flex items-center gap-2 shadow-md transition-all"
                >
                  <Star className="w-4 h-4 md:w-5 md:h-5 text-orangeAccent" />
